...
async function build() {
  ...
  for (const file of svgs) {
    ...
    jsOut = jsOut.replace(new RegExp(`const ${compName} =`), `const ${innerName} =`);
  ...
  const importLines = exports.map((e) => `import ${e.name} from './${e.name}';`);
  const namedExportLine =
    exports.length > 0 ? `export { ${exports.map((e) => e.name).join(", ")} };` : "export {};";
  ...
  const distImportLines = exports.map((e) => `import ${e.name} from './${e.name}.js';`);
  let distIdx = "";
  ...
  distIdx +=
    ...
    "\n\n";
  distIdx +=
    "const icons = { " + exports.map((e) => `${e.name}`).join(", ") + " }\nexport default icons\n";
