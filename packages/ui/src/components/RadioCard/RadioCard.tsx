...
export interface RadioCardProps extends Omit<React.InputHTMLAttributes<HTMLInputElement>, "size"> {
  label: string;
  description?: string;
...
export const RadioCard = React.forwardRef<HTMLInputElement, RadioCardProps>(
  (
    { className, label, description, icon: Icon, footer, checked, variant = "default", ...props },
    ref
  ) => {
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            WebkitMaskImage: "linear-gradient(165deg, white 30%, transparent 80%)",
          }}
        />

        <div className={clsx("flex w-full flex-col", isCompact ? "pl-6" : "pl-6 pt-1")}>
        ...
        {footer && <div className={clsx("mt-3 w-full", isCompact ? "pl-6" : "pl-6")}>{footer}</div>}
      </label>
    );
  }
);
